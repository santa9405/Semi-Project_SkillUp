...
<div class="container" style="margin: auto;">
	...
	<div class="replyWrite">
		<table align="center">
			<tr>
				<td id="replyContentArea"><textArea rows="3" id="replyContent" onkeyup="replyBoxSize(this)"></textArea></td>
				<td id="replyBtnArea">
					<button class="btn btn-outline-secondary" id="addReply">
						등록
					</button>
				</td>
			</tr>
		...
		<ul id="replyListArea">
			...
			<li class="reply-row">
				...
				<div class="replyBtnArea">
					<button class="btn btn-dark" id="updateReply"
						onclick="showUpdateReply(2, this)">수정</button>
					<button class="btn btn-dark" id="deleteReply"
...
var loginMemberId = "${loginMember.memberEmail}";
var parentBoardNo = ${board.documentNo};
		...
		success : function(rList) {
			...
			$.each(rList, function(index, item) {
				...
				/*if(today.getMonth()+1 < 10) {
					var now = today.getFullYear()+ "-0" + (today.getMonth()+1) + "-" + today.getDate() + "-" + today.getHours() + "-" + today.getMinutes();
				}
				else {
				...
				now += dateformat((today.getMonth()+1));
				now += dateformat(today.getDate());
				now += dateformat(today.getHours());
				now += dateformat(today.getMinutes());
				console.log(now);
				
				if(item.replyCreateDate.substring(0, 13)==now.substring(0, 13)) {
					var rDate = $("<p>").addClass("rDate").text(now.substring(14)-item.replyCreateDate.substring(14) + "분전");
				} else if(item.replyCreateDate.substring(0, 10)==now.substring(0, 10)) {
					var rDate = $("<p>").addClass("rDate").text(now.substring(11, 13)-item.replyCreateDate.substring(11, 13) + "시간전");
				} else {
					var rDate = $("<p>").addClass("rDate").text((today.getMonth()+1) + "." + today.getDate());
				}
				...
				if(item.memberId == loginMemberName){
					
					li = $("<li style='background-color:rgb(240, 240, 240)'>").addClass("reply-row");
						
					// 대댓글, 수정, 삭제 버튼 영역
					var replyBtnArea = $("<div>").addClass("replyBtnArea");
					
					// ** 추가되는 댓글에 onclick 이벤트를 부여하여 버튼 클릭 시 수정, 삭제를 수행할 수 있는 함수를 이벤트 핸들러로 추가함. 
					var showUpdate = $("<button>").addClass("btn btn-dark btn-sm ml-1").text("수정").attr("onclick", "showUpdateReply("+item.replyNo+", this)");
					var deleteReply = $("<button>").addClass("btn btn-dark btn-sm ml-1").text("삭제").attr("onclick", "deleteReply("+item.replyNo+")");
					
					replyBtnArea.append(deleteReply).append(showUpdate);
...
$("#addReply").on("click", function(){
	...
	if(loginMemberId == "") {
		...
		if(replyContent.length == 0) { // 로그인 되어있는 경우 댓글 x
...
// 댓글입력창 크기조절
function replyBoxSize(box) {
		box.style.height = "55px";
  	box.style.height = (box.scrollHeight + 12) + "px";
}
...
function showUpdateReply(replyNo, el){
	...
	var beforeContent = $(el).parent().next().next().html();
	console.log(beforeContent);
	// beforContent.replace("ad");
	...
	var textarea = $("<textarea onkeyup='replyBoxSize(this)'>").addClass("replyUpdateContent").attr("rows", "3").val(beforeContent);
...
function updateReply(replyNo, el){
	...
	// 수정된 댓글 내용
	var replyContent = $(el).parent().next().val();
