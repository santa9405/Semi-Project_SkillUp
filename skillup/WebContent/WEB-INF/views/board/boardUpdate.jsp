...
<body>
	...
	<div id="wrapper">
		<div class="container">
			...
			<form action="${contextPath}/board/update?document=${board.documentNo}&page=${param.page}&boardNo=${boardNo}" method="post" role="form" onsubmit="return boardValidate();">
		...
		<c:choose>
			...
			<c:when test="${boardNo==3}">
				...
				<option value="2000">Q&A</option>
						...
						<input id="title" name="title" value="${board.boardTitle}">
				</div>
				
				<div id="content">
					<textarea rows="25" name="content" style="outline:none">${board.boardContent}</textarea>
				</div>
				<br>
				<button type="button" class="btn btn-dark" id="boardviewbtn">뒤로가기</button>
    ...
    $("#boardviewbtn").on("click", function() {
			path = "${contextPath}/board/view?document=${board.documentNo}&page=" + ${param.page} + "&boardNo=${boardNo}";
			location.href = path;
		});
